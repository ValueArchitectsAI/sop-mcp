...
class TestDynamicToolRegistration:
    ...
    async def test_sop_tools_registered_with_descriptive_names(self):
        ...
        assert "run_authoring_new_sop" in tool_names
...
class TestSopToolStart:
    ...
    async def test_returns_dict(self):
        result = await call_tool("run_authoring_new_sop")
        assert isinstance(result, dict)

    @pytest.mark.asyncio
    async def test_returns_sop_name(self):
        result = await call_tool("run_authoring_new_sop")
        assert result["sop_name"] == "authoring_new_sop"
...
class TestSopToolContinue:
    ...
    async def test_returns_next_step_number(self):
        result = await call_tool("run_authoring_new_sop", {"current_step": 1})
    ...
    async def test_returns_is_complete_true_for_last_step(self):
        start = await call_tool("run_authoring_new_sop")
        total = start["total_steps"]
        result = await call_tool("run_authoring_new_sop", {"current_step": total})
    ...
    async def test_error_for_step_beyond_total(self):
        start = await call_tool("run_authoring_new_sop")
        total = start["total_steps"]
        result = await call_tool("run_authoring_new_sop", {"current_step": total + 1})
        assert "error" in result

    @pytest.mark.asyncio
    async def test_error_for_invalid_step_zero(self):
        result = await call_tool("run_authoring_new_sop", {"current_step": 0})
        assert "error" in result
...
class TestSopToolVersionParameter:
    ...
    async def test_explicit_version_returns_matching_sop(self):
        result = await call_tool("run_authoring_new_sop", {"version": "1.0"})
        assert result["sop_version"] == "1.0"

    @pytest.mark.asyncio
    async def test_invalid_version_returns_error(self):
        result = await call_tool("run_authoring_new_sop", {"version": "99.99.99"})
        assert "error" in result
        assert "99.99.99" in result["error"]

    @pytest.mark.asyncio
    async def test_version_with_step_navigation(self):
        result = await call_tool("run_authoring_new_sop", {"version": "1.0", "current_step": 1})
        assert result["sop_version"] == "1.0"
        assert result["current_step"] == 2
