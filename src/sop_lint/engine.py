...
Implements the [Agent SOP format specification][spec] as a set of
rule classes, inspired by ruff's ergonomics: stable codes, configurable
severity, ``select`` / ``ignore`` semantics via a ``sop-lint.toml``
config file.

[spec]: https://github.com/strands-agents/agent-sop/blob/main/rules/agent-sop-format.md
...
- ``SOPMCP0xx`` — sop-mcp strict extras (YAML frontmatter) — opt-in, not
  part of the upstream spec. Enable via ``select = ["SOP", "SOPMCP"]``.

The engine is deliberately stdlib-plus-PyYAML (plus ``tomli`` on
Python < 3.11) so the CLI and the ``sop_mcp`` publish path can share
the exact same rule set without heavy dependencies.
"""
...
if sys.version_info >= (3, 11):
    import tomllib
else:  # pragma: no cover — we require 3.11+ anyway
...
# Top-level ``## Section`` headings permitted by the Agent SOP spec.
...
class _StepHasExpectedOutput:
    ...
    per-step contract for ``done``. Upstream agent-sop doesn't require
